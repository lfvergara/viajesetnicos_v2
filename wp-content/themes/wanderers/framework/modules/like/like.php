...
class WanderersMkdfLike {
	private static $instance;
	
	private function __construct() {
		add_action( 'wp_ajax_wanderers_mkdf_like', array( $this, 'ajax' ) );
		add_action( 'wp_ajax_nopriv_wanderers_mkdf_like', array( $this, 'ajax' ) );
	}
	...
	public function add_like() {
		...
		$title       = esc_html__( 'Like this', 'wanderers' );
		
		if ( isset( $_COOKIE[ 'mkdf-like_' . $post->ID ] ) ) {
			$class = 'mkdf-like liked';
			$title = esc_html__( 'You already like this!', 'wanderers' );
		}
		
		return '<a href="#" class="' . esc_attr($class) . '" id="mkdf-like-' . $post->ID . '-' . $rand_number . '" title="' . $title . '">' . $output . '</a>';
	}
}

if ( ! function_exists( 'wanderers_mkdf_create_like' ) ) {
	function wanderers_mkdf_create_like() {
		WanderersMkdfLike::get_instance();
	}
	
	add_action( 'after_setup_theme', 'wanderers_mkdf_create_like' );
}