...
if ( ! function_exists( 'wanderers_mkdf_set_title_breadcrumbs_type_for_options' ) ) {
	/**
	 * This function set breadcrumbs title type value for title options map and meta boxes
	 */
	function wanderers_mkdf_set_title_breadcrumbs_type_for_options( $type ) {
		$type['breadcrumbs'] = esc_html__( 'Breadcrumbs', 'wanderers' );
		
		return $type;
	}
	
	add_filter( 'wanderers_mkdf_title_type_global_option', 'wanderers_mkdf_set_title_breadcrumbs_type_for_options' );
	add_filter( 'wanderers_mkdf_title_type_meta_boxes', 'wanderers_mkdf_set_title_breadcrumbs_type_for_options' );
}

if ( ! function_exists( 'wanderers_mkdf_custom_breadcrumbs' ) ) {
	/**
	 * Function that renders breadcrumbs
	 */
	function wanderers_mkdf_custom_breadcrumbs() {
		global $post;
		
		$pageid    = wanderers_mkdf_get_page_id();
		$homeLink  = esc_url( home_url( '/' ) );
		$home      = esc_html__( 'Home', 'wanderers' );
		...
		$output = '<div itemprop="breadcrumb" class="mkdf-breadcrumbs ' . esc_attr( $holder_classes ) . '" ' . wanderers_mkdf_get_inline_attr( $holder_styles, 'style', ';' ) . '>';
			...
			if ( is_home() && ! is_front_page() ) {
				...
				if ( is_tag() ) {
					$childContent .= $before . esc_html__( 'Posts tagged ', 'wanderers' ) . '"' . single_tag_title( '', false ) . '"' . $after;
					...
					$childContent    .= $before . esc_html__( 'Articles posted by ', 'wanderers' ) . get_the_author_meta( 'display_name', $author_id ) . $after;
					...
					$childContent .= $before . esc_html__( 'Search results for ', 'wanderers' ) . '"' . get_search_query() . '"' . $after;
					
				} elseif ( is_404() ) {
					$childContent .= $before . esc_html__( 'Error 404', 'wanderers' ) . $after;
				...
				if ( get_query_var( 'paged' ) ) {
					$childContent .= $before . " (" . esc_html__( 'Page', 'wanderers' ) . ' ' . get_query_var( 'paged' ) . ")" . $after;
					
				}
				
				$output .= apply_filters( 'wanderers_mkdf_breadcrumbs_title_child_output', $childContent, $delimiter, $before, $after );
			}
		
		$output .= '</div>';
		
		echo wp_kses( apply_filters( 'wanderers_mkdf_breadcrumbs_title_output', $output ), array(
			'div'  => array(
				'itemprop' => true,
				'id'       => true,
				'class'    => true,
