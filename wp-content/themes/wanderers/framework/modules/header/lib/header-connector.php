...
namespace WanderersMkdf\Modules\Header\Lib;
/**
 * Class WanderersMkdfHeaderConnector
 *
...
class WanderersMkdfHeaderConnector {
	...
	public function connect( $config = array() ) {
		do_action( 'wanderers_mkdf_pre_header_connect' );
		...
		if ( ! empty( $config['affect_content'] ) ) {
			add_filter( 'wanderers_mkdf_content_elem_style_attr', array( $this, 'contentMarginFilter' ) );
		}
		
		if ( ! empty( $config['affect_title'] ) ) {
			add_filter( 'wanderers_mkdf_title_content_padding', array( $this, 'titlePaddingFilter' ) );
		}
		
		do_action( 'wanderers_mkdf_after_header_connect' );
	}
	 ...
	 *
	 * Hooks to wanderers_mkdf_title_content_padding filter
	 *
	 * @return int
	 */
