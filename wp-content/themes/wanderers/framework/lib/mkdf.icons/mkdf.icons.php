...
/*
  Class: WanderersMkdfIconCollections
...
class WanderersMkdfIconCollections
{
    ...
    private function initIconCollections() {
        $this->addIconCollection('font_awesome', new WanderersMkdfIconsFontAwesome("Font Awesome", "fa_icon"));
        $this->addIconCollection('font_elegant', new WanderersMkdfIconsFontElegant("Font Elegant", "fe_icon"));
        $this->addIconCollection('ion_icons', new WanderersMkdfIonIcons("Ion Icons", "ion_icon"));
        $this->addIconCollection('linea_icons', new WanderersMkdfLineaIcons('Linea Icons', 'linea_icon'));
        $this->addIconCollection('linear_icons', new WanderersMkdfLinearIcons('Linear Icons', 'linear_icon'));
        $this->addIconCollection('simple_line_icons', new WanderersMkdfSimpleLineIcons('Simple Line Icons', 'simple_line_icon'));
        $this->addIconCollection('dripicons', new WanderersMkdfDripicons('Dripicons', 'dripicon'));
    }
    ...
    public function getIconsMetaBoxOrOption($attributes) {
        ...
        $options = array(
            'font_awesome'      => esc_html__('Font Awesome', 'wanderers'),
            'font_elegant'      => esc_html__('Font Elegant', 'wanderers'),
            'ion_icons'         => esc_html__('Ion Icons', 'wanderers'),
            'simple_line_icons' => esc_html__('Simple Line Icons', 'wanderers')
        );
        if ($scope == 'regular') {
            $options = array(
                'font_awesome'      => esc_html__('Font Awesome', 'wanderers'),
                'font_elegant'      => esc_html__('Font Elegant', 'wanderers'),
                'ion_icons'         => esc_html__('Ion Icons', 'wanderers'),
                'linea_icons'       => esc_html__('Linea Icons', 'wanderers'),
                'linear_icons'      => esc_html__('Linear Icons', 'wanderers'),
                'simple_line_icons' => esc_html__('Simple Line Icons', 'wanderers'),
                'dripicons'         => esc_html__('Dripicons', 'wanderers')
            );
        }
        ...
        foreach ($icon_collections as $collection_key => $collection_object) {
            ...
            $mkd_icon_pack_container = wanderers_mkdf_add_admin_container(
                array(
                    'parent'          => $parent,
    ...
    public function getVCParamsArray($iconPackDependency = array(), $iconCollectionPrefix = "", $emptyIconPack = false) {
        ...
        $iconPackParams = array(
            'type'        => 'dropdown',
            'heading'     => esc_html__('Icon Pack', 'wanderers'),
        ...
        if (is_array($this->iconCollections) && count($this->iconCollections)) {
            foreach ($this->iconCollections as $key => $collection) {
                $iconSetParams[] = array(
                    'type'        => 'dropdown',
                    'heading'     => esc_html__('Icon', 'wanderers'),
    ...
    public function getIconWidgetParamsArray() {
        ...
        $iconPackParams[] = array(
            'type'    => 'dropdown',
            'name'    => 'icon_pack',
            'title'   => esc_html__('Icon Pack', 'wanderers'),
            'options' => array(
                'font_awesome'      => esc_html__('Font Awesome', 'wanderers'),
                'font_elegant'      => esc_html__('Font Elegant', 'wanderers'),
                'ion_icons'         => esc_html__('Ion Icons', 'wanderers'),
	...
	public function getSocialIconWidgetMultipleParamsArray($count) {
		...
		for ( $n = 1; $n <= $count; $n++ ) {
			...
			if ( is_array( $iconCollectionsVC ) && count( $iconCollectionsVC ) ) {
				foreach ( $iconCollectionsVC as $key => $collection ) {
					$iconOps[] = array(
						...
						'title'   => sprintf( esc_html__( 'Icon %s %s Icon', 'wanderers' ), $n, $collection->title ),
			...
			$iconOps[] = array(
				'type'  => 'textfield',
				'name'  => 'link_'.$n,
				'title' => sprintf( esc_html__( 'Link %s', 'wanderers' ), $n )
			);

			$iconOps[] = array(
				'type'    => 'dropdown',
				'name'    => 'target_'.$n,
				'title'   => sprintf( esc_html__( 'Link Target %s', 'wanderers' ), $n ),
    ...
    public function getIconCollectionIcons(iWanderersMkdfIconCollection $collection) {
    ...
    public function getIconCollectionsEmpty($no_empty_key = "") {
        $array = array();
        $array[$no_empty_key] = esc_html__( 'No Icon', 'wanderers');
        foreach ($this->iconCollections as $key => $collection) {
            $array[$key] = $collection->title;
        }
    ...
    public function getIconCollectionsVCEmpty() {
        $vc_array = array();
	    $vc_array[ esc_html__( 'No Icon', 'wanderers' ) ] = '';
        foreach ($this->iconCollections as $key => $collection) {
            $vc_array[$collection->title] = $key;
        }
    ...
    public function enqueueStyles() {
        if (is_array($this->iconCollections) && count($this->iconCollections)) {
            foreach ($this->iconCollections as $collection_key => $collection_obj) {
                wp_enqueue_style('wanderers-mikado-' . $collection_key, $collection_obj->styleUrl);
            }
        }
    }
...
if (!function_exists('wanderers_mkdf_activate_theme_icons')) {
    function wanderers_mkdf_activate_theme_icons() {
        global $wanderers_mkdf_IconCollections;
        $wanderers_mkdf_IconCollections = WanderersMkdfIconCollections::get_instance();
    }

    add_action('after_setup_theme', 'wanderers_mkdf_activate_theme_icons');
}