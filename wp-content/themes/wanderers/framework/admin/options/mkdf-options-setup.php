...
if ( ! function_exists( 'wanderers_mkdf_admin_map_init' ) ) {
	function wanderers_mkdf_admin_map_init() {
		do_action( 'wanderers_mkdf_before_options_map' );
		...
		do_action( 'wanderers_mkdf_options_map' );
		
		do_action( 'wanderers_mkdf_after_options_map' );
	}
	
	add_action( 'after_setup_theme', 'wanderers_mkdf_admin_map_init', 1 );
}