(function($){
			...
			frame: function () {
				...
				this._frame.on('update', function () {
					...
					jQuery.ajax({
						type: "post",
						url: ajaxurl,
						data: "action=wanderers_mkdf_gallery_upload_get_images&ids=" + ids,
	...
	function mkdfInitAjaxForm() {
		...
		$(document).delegate(".mkdf_ajax_form", "submit", function (a) {
			var b = $(this),
				c = {
					action: "wanderers_mkdf_save_options"
				};
			...
			mkdfImportBtn.on('click', function (e) {
				...
				if (confirm(confirmMessage)) {
					...
					if (import_opt === 'content') {
						for (var i = 1; i < 10; i++) {
							var str = i < 10 ? 'wanderers_content_0' + i + '.xml' : 'wanderers_content_' + i + '.xml';
						...
						for (var i = 1; i < 10; i++) {
							var str = i < 10 ? str = 'wanderers_content_0' + i + '.xml' : 'wanderers_content_' + i + '.xml';
