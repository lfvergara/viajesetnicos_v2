...
class ToursBookingTable extends WP_List_Table {
	...
	public function __construct() {
		parent::__construct(
			array(
				'plural'   => esc_html__('Bookings', 'mkdf-tours'),
				'singular' => esc_html__('Booking', 'mkdf-tours'),
				'ajax'     => false
			)
		);
	}
	...
	public function column_default($item, $column_name) {
		switch($column_name) {
			case 'booking_info':
				...
				?>
				<p>#<?php echo esc_html($item['id']); ?> <?php esc_html_e('by','mkdf-tours'); ?> <?php echo esc_html($name); ?></p>
			...
			case 'amount':
				...
				$amountLabel = $amount === 1 ? esc_html__('ticket', 'mkdf-tours') : esc_html__('tickets', 'mkdf-tours');
			...
			case 'actions':
				...
				$buttons = array(
					'approved' => '<a href="#" class="mkdf-booking-table-action-btn approve-booking" data-booking-id="'.$item_id.'" title="'.esc_attr__('Approve', 'mkdf-tours').'"><i class="dashicons dashicons-yes"></i></a>',
					'canceled' => '<a href="#" class="mkdf-booking-table-action-btn cancel-booking" data-booking-id="'.$item_id.'" title="'.esc_attr__('Cancel', 'mkdf-tours').'"><i class="dashicons dashicons-no-alt"></i></a>'
				);
	...
	function get_columns() {
		$columns = array(
			...
			'booking_info'   => esc_html__('Booking Info', 'mkdf-tours'),
			'tour_id'        => esc_html__('Tour Name', 'mkdf-tours'),
			'booking_date'   => esc_html__('Date', 'mkdf-tours'),
			'amount'         => esc_html__('Amount', 'mkdf-tours'),
			'price'          => esc_html__('Price', 'mkdf-tours'),
			'status'         => esc_html__('Booking Status', 'mkdf-tours'),
			'payment_status' => esc_html__('Payment Status', 'mkdf-tours'),
			'user_message'   => esc_html__('User Message', 'mkdf-tours'),
			'actions'        => esc_html__('Actions', 'mkdf-tours'),
		);
