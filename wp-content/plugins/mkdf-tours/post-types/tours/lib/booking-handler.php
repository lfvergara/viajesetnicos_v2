 ...
 * @package MikadoTours\CPT\Tours\Lib
 */
class BookingHandler {
	/**
	...
	public function handleBooking() {
		...
		if(!$returnObject->status) {
			$returnObject->messages[] = esc_html__('There was some booking error. Please try again later', 'mkdf-tours');
		} else {
			$returnObject->messages[] = esc_html__('Booking successful', 'mkdf-tours');

			if(mkdf_tours_theme_installed()) {
				$redirectOptionValue = wanderers_mkdf_options()->getOptionValue('tours_checkout_page');
	...
	public function handleAvailability() {
		...
		if(!$price) {
			$returnObject->status     = false;
			$returnObject->messages[] = esc_html__('Tour doesn\'t have price defined', 'mkdf-tours');
	...
	private function checkAvailability($fields) {
		...
		if(!$returnObject->available) {
			$ticketsLabel          = $numberOfAvailableBookings === 1 ? esc_html__('ticket', 'mkdf-tours') : esc_html__('tickets', 'mkdf-tours');
			$returnObject->message = esc_html__('We have', 'mkdf-tours').' '.$numberOfAvailableBookings.' '.$ticketsLabel.' '.esc_html__('left', 'mkdf-tours');
		} else {
			$returnObject->message = esc_html__('Booking available', 'mkdf-tours');
		}
	...
	public function loadAssets() {
		if(is_singular('tour-item')) {
			...
			wp_enqueue_script('wanderers-mkdf-tour-booking', plugins_url(MIKADO_TOURS_REL_PATH.'/assets/js/tour-booking.js'), $deps, false, true);
		}
	}
	...
	public function localizeTourData() {
		if(is_singular('tour-item')) {
			...
			wp_localize_script('wanderers-mkdf-tour-booking', 'mkdfTourData', array(
				'data'    => array(
					'id'             => $id,
	...
	private function setLabels() {
		$this->labels = array(
			'name'             => esc_html__('Please enter your name', 'mkdf-tours'),
			'email'            => esc_html__('Please enter valid email', 'mkdf-tours'),
			'confirmEmail'     => esc_html__('Email and confirmation email field aren\'t the same', 'mkdf-tours'),
			'date'             => esc_html__('Please choose date', 'mkdf-tours'),
			'dateNotAvailable' => esc_html__('Chosen date is not available', 'mkdf-tours'),
			'time'             => esc_html__('Please choose time', 'mkdf-tours'),
			'numberOfTickets'  => esc_html__('Please choose number of tickets', 'mkdf-tours')
		);
	}
	...
	private function sendUserConfirmationEmail($id, $bookingEmail, $bookingHash) {
		...
		$message = esc_html__('We have received your reservation for','mkdf-tours') .  ' '. $tour_title . ' '. esc_html__('Your booking will be complete upon payment.', 'mkdf-tours');

		$subject  = esc_html__('Booking Information For', 'mkdf-tours') . ' '. $tour_title . ' '. esc_html__('on', 'mkdf-tours') . ' ' . $name;
	...
	private function sendAdminConfirmationEmail($id, $bookingEmail, $bookingHash) {
		...
		if(wanderers_mkdf_options()->getOptionValue('enable_admin_booking_email') == 'yes'){
			//Get email address
			if(wanderers_mkdf_options() -> getOptionValue('admin_email') != '') {
				$mail_to = wanderers_mkdf_options() -> getOptionValue('admin_email');
			} else {
			...
			$message = esc_html__('You have received the reservation for','mkdf-tours') .  ' '. $tour_title . ' .';
