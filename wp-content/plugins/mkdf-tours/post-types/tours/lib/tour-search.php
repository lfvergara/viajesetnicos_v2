...
class TourSearch {
	...
	public function getResults($count = false) {
		...
		switch($this->orderBy) {
			case 'date':
				$orderTableField = "{$wpdb->prefix}posts.post_date";
				break;
			case 'price':
	...
	private function setPropertiesFromRequest() {
		...
		if(mkdf_tours_theme_installed()) {
            ...
            $defaultViewType = wanderers_mkdf_get_meta_field_intersect('tours_search_default_view_type',$pageId );

			$orderingOption = wanderers_mkdf_options()->getOptionValue('tours_search_default_ordering');
		...
		if(mkdf_tours_theme_installed()) {
			$this->toursPerPage = wanderers_mkdf_options()->getOptionValue('tours_per_page');
