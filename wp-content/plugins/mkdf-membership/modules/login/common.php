	...
	function mkdf_membership_set_social_vars() {
		...
		if ( $social_login_enabled ) {
			...
			$social_variables = array(
				'facebookAppId'  => wanderers_mkdf_options()->getOptionValue( 'enable_facebook_social_login' ) == 'yes' ? wanderers_mkdf_options()->getOptionValue( 'enable_facebook_login_fbapp_id' ) : null,
				'googleClientId' => wanderers_mkdf_options()->getOptionValue( 'enable_google_social_login' ) == 'yes' ? wanderers_mkdf_options()->getOptionValue( 'enable_google_login_client_id' ) : null
			);
			
			wp_localize_script( 'wanderers-mikado-membership-script', 'mkdfSocialLoginVars', array(
	...
	function mkdf_membership_register_user_from_social_network( $params ) {
		...
		if ( wp_verify_nonce( $nonce, 'mkdf_validate_' . $network . '_login' ) ) {
			...
			$userdata = array(
				...
				'user_email'   => $email,
				'user_pass'    => $password,
                'role'         => apply_filters('mkdf_membership_default_role', get_option('default_role'))
			);
