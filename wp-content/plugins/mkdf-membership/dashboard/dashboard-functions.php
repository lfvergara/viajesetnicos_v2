...
if ( ! function_exists( 'mkdf_membership_get_my_account_page_url' ) ) {
	...
	function mkdf_membership_get_my_account_page_url() {
		...
		if ( mkdf_membership_theme_installed() && wanderers_mkdf_is_woocommerce_installed() ) {
...
if ( ! function_exists( 'mkdf_membership_get_redirect_url' ) ) {
	...
	function mkdf_membership_get_redirect_url() {
		...
		if ( $url == '' && wanderers_mkdf_is_woocommerce_installed() ) {
			$my_account_page_id = get_option( 'woocommerce_myaccount_page_id' );
...
if ( ! function_exists( 'mkdf_membership_get_dashboard_pages' ) ) {
	...
	function mkdf_membership_get_dashboard_pages() {
		...
		if( $action == 'profile' ) {
            $page = mkdf_membership_get_dashboard_template_part( 'profile', '', $params );
        } else if( $action == 'edit-profile' ) {
            $page = mkdf_membership_get_dashboard_template_part( 'edit-profile', '', $params );
        }

        $page = apply_filters( 'mkdf_membership_dashboard_pages', $page, $action );

		//Include template part
		if ( $page != '' ) {
			$html = $page;
		} else {
			$html = mkdf_membership_get_dashboard_template_part( 'profile', '', $params );
		}
...
if ( ! function_exists( 'mkdf_membership_get_dashboard_navigation_items' ) ) {
	...
	function mkdf_membership_get_dashboard_navigation_items() {
		...
		$account_url   = mkdf_membership_get_my_account_page_url();
		
		$items = array(
			'account'      => array(
				'url'           => esc_url($account_url),
				'text'          => esc_html__( 'Account', 'mkdf-membership'),
