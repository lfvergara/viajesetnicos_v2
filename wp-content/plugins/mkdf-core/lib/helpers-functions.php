...
if ( ! function_exists( 'mkdf_core_get_cpt_shortcode_module_template_part' ) ) {
	 ...
	 * @return html
	 */
	function mkdf_core_get_cpt_shortcode_module_template_part( $post_type, $shortcode, $template, $slug = '', $params = array(), $additional_params = array() ) {
...
if ( ! function_exists( 'mkdf_core_get_module_shortcode_template_part' ) ) {
    /**
     * Loads module template part.
     *
     * @param string $module name of the module to load
     ...
     * @return html
     */
    function mkdf_core_get_module_shortcode_template_part( $module, $shortcode, $template, $slug = '', $params = array() ) {
...
if ( ! function_exists( 'wanderers_mkdf_add_user_custom_fields' ) ) {
	/**
	 * Function creates custom social fields for users
	 *
	 * return $user_contact
	 */
	function wanderers_mkdf_add_user_custom_fields( $user_contact ) {
		/**
	...
	add_filter( 'user_contactmethods', 'wanderers_mkdf_add_user_custom_fields' );
}
...
if ( ! function_exists( 'mkdf_core_set_open_graph_meta' ) ) {
	...
	function mkdf_core_set_open_graph_meta() {
		...
		if ( wanderers_mkdf_option_get_value( 'enable_open_graph' ) === 'yes' ) {
			...
			if ( wanderers_mkdf_is_woocommerce_installed() && ( function_exists( 'is_shop' ) && is_shop() ) ) {
			...
			if ( ! empty( $id ) ) {
				...
				if ( get_post_thumbnail_id( $id ) !== '' ) {
					...
					$image = wanderers_mkdf_option_get_value( 'open_graph_image' );
				}
			} else {
				global $wp;
	...
	add_action( 'wanderers_mkdf_header_meta', 'mkdf_core_set_open_graph_meta' );
}


/* Function for adding custom meta boxes hooked to default action */
if ( class_exists( 'WP_Block_Type' ) && defined( 'MIKADO_ROOT' ) ) {
	add_action( 'admin_head', 'wanderers_mkdf_meta_box_add' );
} else {
	add_action( 'add_meta_boxes', 'wanderers_mkdf_meta_box_add' );
}

if (!function_exists('wanderers_mikado_create_meta_box_handler')) {
	function wanderers_mikado_create_meta_box_handler( $box, $key, $screen ) {
		add_meta_box(
			'mkdf-meta-box-' . $key,
			$box->title,
			'wanderers_mkdf_render_meta_box',
			$screen,
			'advanced',
			'high',
			array( 'box' => $box )
