...
class MikadoCoreImport {
	...
	public function import_action() {
						...
						case 'content':
							...
							if($content_start) {
								if (!MikadoCoreDashboard::get_instance()->check_purchase_code($_POST['options']['demo'])) {
									mkdf_core_ajax_status('error', esc_html__('Please don\'t try to hack me. Purchase code registered is not valid', 'mkdf-core'));
									exit;
								}
							}
			...
			mkdf_core_ajax_status($this->get_status(), $this->get_message(), $this->get_data());
		}
		wp_die();
	}
	...
	public function import_widgets($demo) {
		...
		if($widgets_content) {
			foreach ((array)$widgets_content['widgets'] as $wanderers_widget_id => $wanderers_widget_data) {
				update_option('widget_' . $wanderers_widget_id, $wanderers_widget_data);
			}
	...
	public function import_sidebars_widgets( $file ) {
		$wanderers_sidebars = get_option( "sidebars_widgets" );
		unset( $wanderers_sidebars['array_version'] );
		$data = $this->unserialized_content( $file );

		if ( $data && is_array( $data['sidebars'] ) ) {
			$wanderers_sidebars = array_merge( (array) $wanderers_sidebars, (array) $data['sidebars'] );
			unset( $wanderers_sidebars['wp_inactive_widgets'] );
			$wanderers_sidebars                  = array_merge( array( 'wp_inactive_widgets' => array() ), $wanderers_sidebars );
			$wanderers_sidebars['array_version'] = 2;
			wp_set_sidebars_widgets( $wanderers_sidebars );
			return true;
		} else {
			return false;
		}
...
//		}
//
		if(mkdf_core_is_woocommerce_installed()) {
		...
		$wanderers_import = new WP_Import();
		set_time_limit( 0 );

		$wanderers_import->fetch_attachments = $attachments;
		$returned_value                  = $wanderers_import->import( $file . $xml );
	...
	public function populate_single_pages() {
		...
		if ( isset( $_POST ) && !empty( $_POST ) && !empty($_POST['options']['demo']) ) {
			if ( wp_verify_nonce( $_POST['options']['nonce'], 'mkdf_cd_import_nonce' ) ) {
				...
				if($pages){
					mkdf_core_ajax_status( 'success', '', $html);
				} else {
					mkdf_core_ajax_status( 'error', esc_html__( 'Pages don\'t exist', 'mkdf-core' ), '');
				}
			}
		}
