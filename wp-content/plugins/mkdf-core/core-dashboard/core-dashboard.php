...
if(!class_exists('MikadoCoreDashboard')){
	...
	class MikadoCoreDashboard {
		...
		public	$licence_field = 'wanderers_purchase_info';
		public	$import_field = 'wanderers_import_params';
		...
		function dashboard_add_page() {
			...
			if (mkdf_core_theme_installed()) {
				...
				$page = add_menu_page(
					esc_html__('Wanderers Dashboard', 'mkdf-core'),
					esc_html__('Wanderers Dashboard', 'mkdf-core'),
					'administrator',
					'mkdf_core_dashboard',
		...
		function enqueue_styles(){
			wp_enqueue_style( 'mkdf-core-dashboard-style', plugins_url( MIKADO_CORE_REL_PATH . '/core-dashboard/assets/css/core-dashboard.min.css' ));
		}

		function enqueue_scripts(){
			wp_enqueue_script( 'select2', MIKADO_FRAMEWORK_ROOT . '/admin/assets/js/select2.min.js', array(), false, true );
			wp_enqueue_script( 'mkdf-core-dashboard-script', plugins_url( MIKADO_CORE_REL_PATH . '/core-dashboard/assets/js/modules/core-dashboard.js' ), array(), false, true );
		...
		function register_purchase_code() {
			...
			if ( empty( $data['purchase_code'] ) || empty( $data['email'] ) ) {
				mkdf_core_ajax_status( 'error', esc_html__( 'Purchase Code and Email are empty', 'mkdf-core' ), array(
					'purchase_code' => false,
					'email'         => false
				) );
			} elseif ( empty( $data['purchase_code'] ) ) {
				mkdf_core_ajax_status( 'error', esc_html__( 'Purchase Code is empty', 'mkdf-core' ), array( 'purchase_code' => false ) );
			} elseif ( empty( $data['email'] ) ) {
				mkdf_core_ajax_status( 'error', esc_html__( 'Email is empty', 'mkdf-core' ), array( 'email' => false ) );
			}
			...
			if ( isset( $json['success'] ) && $json['success'] ) {
				...
				mkdf_core_ajax_status( 'success', $this->response_codes( $json['response_code'] ) );

			} elseif ( isset( $json['message'] ) && ! $json['success'] && ( isset( $json['data']['error'] ) && $json['data']['error'] == 404 ) ) {

				mkdf_core_ajax_status( 'error', $this->response_codes( $json['response_code'] ), array( 'purchase_code' => false ) );

			} elseif ( isset( $json['message'] ) && ! $json['success'] && ( isset( $json['data']['error'] ) && $json['data']['error'] == 'used' ) ) {

				mkdf_core_ajax_status( 'error', $this->response_codes( $json['response_code'] ), array( 'already_used' => true ) );

			} elseif ( isset( $json['message'] ) && ! $json['success'] ) {

				mkdf_core_ajax_status( 'error', $this->response_codes( $json['response_code'] ) );
			}
		}
		...
		function response_codes( $code ) {
			...
			switch ( $code ):
				...
				case 604:
					...
					break;
				case 650:
					$message = esc_html__( 'Code is already registered for other domain', 'mkdf-core' );
					break;
				case 651:
