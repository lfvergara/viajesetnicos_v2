...
class Separator implements Lib\ShortcodeInterface {
	...
	private function getHolderStyles( $params ) {
		...
		if ( $params['width'] !== '' ) {
			if ( wanderers_mkdf_string_ends_with( $params['width'], '%' ) || wanderers_mkdf_string_ends_with( $params['width'], 'px' ) ) {
				$styles[] = 'width: ' . $params['width'];
			} else {
				$styles[] = 'width: ' . wanderers_mkdf_filter_px( $params['width'] ) . 'px';
			}
		}
		
		if ( $params['thickness'] !== '' ) {
			$styles[] = 'border-bottom-width: ' . wanderers_mkdf_filter_px( $params['thickness'] ) . 'px';
		}
		
		if ( $params['top_margin'] !== '' ) {
			if ( wanderers_mkdf_string_ends_with( $params['top_margin'], '%' ) || wanderers_mkdf_string_ends_with( $params['top_margin'], 'px' ) ) {
				$styles[] = 'margin-top: ' . $params['top_margin'];
			} else {
				$styles[] = 'margin-top: ' . wanderers_mkdf_filter_px( $params['top_margin'] ) . 'px';
			}
		}
		
		if ( $params['bottom_margin'] !== '' ) {
			if ( wanderers_mkdf_string_ends_with( $params['bottom_margin'], '%' ) || wanderers_mkdf_string_ends_with( $params['bottom_margin'], 'px' ) ) {
				$styles[] = 'margin-bottom: ' . $params['bottom_margin'];
			} else {
				$styles[] = 'margin-bottom: ' . wanderers_mkdf_filter_px( $params['bottom_margin'] ) . 'px';
			}
		}
