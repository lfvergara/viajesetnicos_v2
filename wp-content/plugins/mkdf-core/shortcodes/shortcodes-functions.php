...
if ( ! function_exists( 'mkdf_core_add_admin_shortcodes_styles' ) ) {
	...
	function mkdf_core_add_admin_shortcodes_styles() {
		...
		wp_enqueue_style( 'wanderers-mikado-core-vc-shortcodes', MIKADO_CORE_ASSETS_URL_PATH . '/css/admin/mkdf-vc-shortcodes.css' );
	}
	
	add_action( 'wanderers_mkdf_admin_scripts_init', 'mkdf_core_add_admin_shortcodes_styles' );
}
...
if ( ! function_exists( 'mkdf_core_add_admin_shortcodes_custom_styles' ) ) {
	...
	function mkdf_core_add_admin_shortcodes_custom_styles() {
		...
		if ( ! empty( $style ) ) {
			wp_add_inline_style( 'wanderers-mikado-core-vc-shortcodes', $style );
		}
	}
	
	add_action( 'wanderers_mkdf_admin_scripts_init', 'mkdf_core_add_admin_shortcodes_custom_styles' );
}