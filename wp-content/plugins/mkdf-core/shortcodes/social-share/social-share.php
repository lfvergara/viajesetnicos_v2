...
class SocialShare implements Lib\ShortcodeInterface {
	...
	public function render( $atts, $content = null ) {
		...
		$params['enable_social_share'] = ( wanderers_mkdf_options()->getOptionValue( 'enable_social_share' ) == 'yes' ) ? true : false;
		
		//Is social share enabled for post type
		$post_type         = get_post_type();
		$params['enabled'] = ( wanderers_mkdf_options()->getOptionValue( 'enable_social_share_on_' . $post_type ) == 'yes' ) ? true : false;
	...
	private function getSocialNetworksParams( $params ) {
		...
		foreach ( $this->socialNetworks as $net ) {
			...
			if ( wanderers_mkdf_options()->getOptionValue( 'enable_' . $net . '_share' ) == 'yes' ) {
				...
				$params['custom_icon'] = ( wanderers_mkdf_options()->getOptionValue( $net . '_icon' ) ) ? wanderers_mkdf_options()->getOptionValue( $net . '_icon' ) : '';
    ...
    private function getSocialNetworkShareLink($net, $image) {
        switch ($net) {
            ...
            case 'twitter':
                ...
                $twitter_via = (wanderers_mkdf_options()->getOptionValue('twitter_via') !== '') ? ' via ' . wanderers_mkdf_options()->getOptionValue('twitter_via') . ' ' : '';
                $link =  'window.open(\'https://twitter.com/intent/tweet?text=' . urlencode( wanderers_mkdf_the_excerpt_max_charlength( $count_char ) . $twitter_via ) . ' ' . get_permalink() . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');';
                break;
            case 'linkedin':
