...
if ( ! function_exists( 'wanderers_mkdf_get_social_share_html' ) ) {
	/**
	 ...
	 * @return mixed|string
	 */
	function wanderers_mkdf_get_social_share_html( $params = array() ) {
		if ( wanderers_mkdf_core_plugin_installed() ) {
			return wanderers_mkdf_execute_shortcode( 'mkdf_social_share', $params );
		}
	}
}

if ( ! function_exists( 'wanderers_mkdf_the_excerpt_max_charlength' ) ) {
	/**
	 ...
	 */
	function wanderers_mkdf_the_excerpt_max_charlength( $charlength ) {
		$twitter_via_meta = wanderers_mkdf_options()->getOptionValue( 'twitter_via' );
