...
class Icon implements Lib\ShortcodeInterface {
		    ...
		    vc_map(
		    	array(
				    ...
				    'params'                    => array_merge(
					    ...
					    \WanderersMkdfIconCollections::get_instance()->getVCParamsArray(),
					    array(
						    array(
							    'type'       => 'dropdown',
							    'param_name' => 'size',
	...
	private function generateIconStyles( $params ) {
		...
		if ( ( $params['type'] !== 'mkdf-normal' && ! empty( $params['shape_size'] ) ) || ( $params['type'] == 'mkdf-normal' ) ) {
			if ( ! empty( $params['custom_size'] ) ) {
				$iconStyles[] = 'font-size:' . wanderers_mkdf_filter_px( $params['custom_size'] ) . 'px';
			}
		}
	...
	private function generateIconHolderStyles( $params ) {
		...
		if ( $params['type'] !== 'mkdf-normal' ) {
			...
			if ( ! empty( $shapeSize ) ) {
				$iconHolderStyles[] = 'width: ' . wanderers_mkdf_filter_px( $shapeSize ) . 'px';
				$iconHolderStyles[] = 'height: ' . wanderers_mkdf_filter_px( $shapeSize ) . 'px';
				$iconHolderStyles[] = 'line-height: ' . wanderers_mkdf_filter_px( $shapeSize ) . 'px';
			}
			...
			if ( ! empty( $params['border_color'] ) && ( isset( $params['border_width'] ) && $params['border_width'] !== '' ) ) {
				...
				$iconHolderStyles[] = 'border-width: ' . wanderers_mkdf_filter_px( $params['border_width'] ) . 'px';
			...
			if ( $params['type'] == 'mkdf-square' ) {
				if ( isset( $params['border_radius'] ) && $params['border_radius'] !== '' ) {
					$iconHolderStyles[] = 'border-radius: ' . wanderers_mkdf_filter_px( $params['border_radius'] ) . 'px';
				}
			}
		}
