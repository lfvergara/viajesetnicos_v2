...
class MikadoInstagramWidget extends WP_Widget {
	...
	public function widget( $args, $instance ) {
		...
		if ( is_array( $images_array ) && count( $images_array ) ) { ?>
			<ul class="mkdf-instagram-feed clearfix mkdf-col-<?php echo esc_attr( $number_of_cols ); ?> <?php echo esc_attr( $widget_class ); ?>" <?php echo wanderers_mkdf_get_inline_attrs( $slider_data ); ?>>
				<?php
				foreach ( $images_array as $image ) { ?>
					<li> 
						<span class="mkdf-instagram-image">
							<a href="<?php echo esc_url( $instagram_api->getHelper()->getImageLink( $image ) ); ?>" target="_blank">
								<?php echo wanderers_mkdf_kses_img( $instagram_api->getHelper()->getImageHTML( $image ) ); ?>
							</a>
						</span>
					</li>
				<?php } ?>
